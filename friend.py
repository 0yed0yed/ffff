...
DEFAULT_REQUESTS_PER_MINUTE = 550

# إعداد مجلد مؤقت لـ Vercel
if os.environ.get('VERCEL'):
    tempfile.tempdir = '/tmp'

def load_protobuf_classes():
    classes = {}
    # استخدام مجلد ثابت لـ Vercel
    if os.environ.get('VERCEL'):
        temp_dir = '/tmp/proto_classes'
...
def generate_access_token(uid, password):
    ...
    try:
        response = requests.post(OAUTH_URL, headers=headers, data=data, timeout=8, verify=False)
...
def major_login(open_id, access_token):
    ...
    try:
        response = requests.post(MAJOR_LOGIN_URL, data=encrypted_payload, headers=BASE_HEADERS, timeout=8, verify=False)
        if response.status_code == 200:
            res = MajorLoginRes()
...
def balanced_spammer(target_uid, region=None, max_failures=10, retry_minutes=5):
    accounts = load_accounts()
    if not accounts:
        return
    
    THREADS = 1
    DELAY = 60.0 / DEFAULT_REQUESTS_PER_MINUTE
    RETRY_DELAY_SECONDS = retry_minutes * 60
            ...
            if token:
                ...
                if not request_success:
                    ...
                    if consecutive_failures >= max_failures:
                        with sessions_lock:
                            if target_uid in active_sessions:
                                active_sessions[target_uid]["waiting"] = True
                        
                        wait_until = time.time() + RETRY_DELAY_SECONDS
                        ...
                        with sessions_lock:
                            if target_uid in active_sessions:
                                active_sessions[target_uid]["waiting"] = False
...
def start_spamming():
    ...
    max_failures = request.args.get('max_failures', DEFAULT_MAX_FAILURES, type=int)
    retry_minutes = request.args.get('retry_minutes', DEFAULT_RETRY_MINUTES, type=int)
    ...
    stats = {
        ...
        "retry_settings": {
            ...
            "retry_delay_minutes": retry_minutes
        },
        "speed": {
            "requests_per_minute": DEFAULT_REQUESTS_PER_MINUTE,
            "delay_seconds": round(60.0 / DEFAULT_REQUESTS_PER_MINUTE, 3)
        }
    }
    ...
    thread = threading.Thread(target=balanced_spammer, args=(target_uid, region, max_failures, retry_minutes))
    thread.daemon = True
    thread.start()
    ...
    return jsonify({
        ...
        "threads": 1,
        "delay": f"{round(60.0/DEFAULT_REQUESTS_PER_MINUTE, 3)}s",
        "estimated_speed": f"~{DEFAULT_REQUESTS_PER_MINUTE} requests/minute",
...
def get_status():
    with sessions_lock:
        ...
        for uid, data in active_sessions.items():
            ...
            success_rate = round((data["stats"]["success"] / data["stats"]["total"] * 100), 1) if data["stats"]["total"] > 0 else 0
            ...
            sessions_info.append({
                ...
                "is_waiting": data.get("waiting", False)
            })
...
def set_speed():
    global DEFAULT_REQUESTS_PER_MINUTE
    rpm = request.args.get('rpm', type=int)
    if not rpm:
        return jsonify({"error": "Missing 'rpm' parameter", "current": DEFAULT_REQUESTS_PER_MINUTE}), 400
    
    if rpm < 50 or rpm > 1000:
        return jsonify({"error": "Speed must be between 50 and 1000 RPM"}), 400
    
    old = DEFAULT_REQUESTS_PER_MINUTE
    ...
    return jsonify({
        ...
        "old_speed_rpm": old,
        "new_speed_rpm": rpm,
        "new_delay_seconds": round(60.0 / rpm, 3),
        "note": "هذا الإعداد يؤثر فقط على الجلسات الجديدة"
    })
